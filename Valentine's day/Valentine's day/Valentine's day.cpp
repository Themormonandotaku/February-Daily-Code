...
int main(int argc, char **argv) {
	...
	al_clear_to_color(al_map_rgb(450, 0, 640));
	...
	//change the numbers to change the background color, 000 is black, 111 is white
	...
	while (1)
	{
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			al_clear_to_color(al_map_rgb(600, t, 400));
			al_set_target_bitmap(al_get_backbuffer(display));
